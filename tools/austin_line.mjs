// Convert the TUMFTM Spielberg CSV into src/world/circuits/austinLine.ts
//   node tools/austin_line.mjs [startIndex] [--analyse]   (the committed file uses 1036)
// x = east, z = −y (south), centred on the bounding box, index rotated so s = 0 is
// on the short run from Turn 19 to Turn 20 (the pit lane along the main straight never wraps).
// The data is already in racing order (anticlockwise).
import fs from 'node:fs';

const src = fs.readFileSync(new URL('../assets-src/Austin.csv', import.meta.url), 'utf8');
const rows = src.split('\n').filter((l) => l.trim() && !l.startsWith('#')).map((l) => l.split(',').map(Number));
let pts = rows.map(([x, y, wr, wl]) => ({ x, z: -y, wr, wl }));
const start = Number(process.argv[2] ?? 1036);
...
const out = `/**
 * Circuit of the Americas, Austin — surveyed centreline (the Grand Prix layout).
 *
 ...
 * centred on the circuit's bounding box. Index 0 sits on the short run from
 * Turn 19 to Turn 20, so the pit lane along the main straight never wraps.
 * Generated by tools/austin_line.mjs.
 */
// prettier-ignore
export const AUSTIN_LINE: number[] = [
${lines.join('\n')}
];
`;
fs.writeFileSync(new URL('../src/world/circuits/austinLine.ts', import.meta.url), out);
